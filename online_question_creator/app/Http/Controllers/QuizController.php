...
class QuizController extends Controller
{
    ...
    public function setQuizQuestion(Request $request){
        if ($request->session()->has('quiztitle')) {
            return view('user.setquizquestion');
        }else{
    ...
    public function saveQuestionLocally(Request $request){
        ...
        $ar = $validator->get('opt.*');
        for ($i=0; $i < sizeof($validator->get('opt.*')); $i++) { 
            for ($j=0; $j < sizeof($ar["opt.".$i]) ; $j++) { 
                echo $ar["opt.".$i][$j].'<br/>';   
            }
        }
        return; */
    ...
    public function showAllExams(Request $request){
        ...
        foreach ($questoin_set as  $qsn) {
        	...
        	//echo $no_of_question;
        	//echo '<br/>';		 
            $question_arr[$qsn->question_set_id] = [$qsn->question_set_title,$no_of_question];
        }  
    ...
    public function startExam(Request $request, $examtitle, $examid){
    	...
    	/*
    	$question_array = [
    		'Question_id'=> ['question', 'question_type', ['option1','option2','option3','option4'] ]
    	]; */
    	$question_array = [];
    	//return $examtitle.' '.$examid;
